...
class PSDSchurSolverTest : public ::testing::Test {
 protected:
  ...
  RelativeReprojectionError createErrorTerm(
      VariableKey<Tangent::SE3> hostK, VariableKey<Tangent::SE3> targetK,
      VariableKey<Tangent::InverseDepth> dinvK, Eigen::Vector2d bearing) {
  ...
  Tangent::GaussianPrior<
      ...
      Tangent::VariableGroup<Tangent::SE3, Tangent::InverseDepth,
                            Tangent::SimpleScalar, DifferentSimpleScalar>>
      prior;

  Tangent::VariableContainer<Tangent::SE3, Tangent::InverseDepth,
                            Tangent::SimpleScalar, DifferentSimpleScalar>
      variableContainer;

  Tangent::ErrorTermContainer<RelativeReprojectionError, DifferenceErrorTerm>
      errorTermContainer;

  Tangent::HuberLossFunction<double> lossFunction =
      Tangent::HuberLossFunction<double>(1000);

  using Solver = Tangent::PSDSchurSolver<
      Scalar<double>, Tangent::LossFunction<Tangent::HuberLossFunction<double>>,
      Tangent::ErrorTermGroup<RelativeReprojectionError, DifferenceErrorTerm>,
      Tangent::VariableGroup<Tangent::SE3, Tangent::InverseDepth,
                            Tangent::SimpleScalar, DifferentSimpleScalar>,
      Tangent::VariableGroup<Tangent::InverseDepth, DifferentSimpleScalar>>;

  Solver solver = Solver(lossFunction);

  using Marginalizer = Tangent::Marginalizer<
      Tangent::Scalar<double>,
      Tangent::VariableGroup<Tangent::SE3, Tangent::InverseDepth,
                            Tangent::SimpleScalar, DifferentSimpleScalar>,
      Tangent::ErrorTermGroup<RelativeReprojectionError, DifferenceErrorTerm>>;
  ...
  Tangent::SE3 hostPose;
  Tangent::SE3 targetPose;
  Tangent::SimpleScalar unobservableScalar = Tangent::SimpleScalar(10);
  // uncorrelated variables.
  DifferentSimpleScalar uncorrelatedScalar = DifferentSimpleScalar(20);
  Tangent::InverseDepth landmark1 = Tangent::InverseDepth(1);
  Tangent::InverseDepth landmark2 = Tangent::InverseDepth(0.75);
  Tangent::InverseDepth landmark3 = Tangent::InverseDepth(0.5);
...
TEST_F(PSDSchurSolverTest, IterateWithOnlyPrior) {
  ...
  EXPECT_EQ(prior.A0.getRowMap<Tangent::SE3>().count(targetKey), 0);
  ...
  EXPECT_EQ(solver.totalDimension, previousDimension - Tangent::SE3::dimension);
  EXPECT_NEAR(solver.dx.block(0, 0, solver.totalDimension, 1).norm(), 0, 1e-9);
}
...
TEST_F(PSDSchurSolverTest, SolveSmallSlamProblemLM) {
  ...
  EXPECT_TRUE(
      marginalizer.marginalizeVariable(hostKey, prior, errorTermContainer,
                                       VariableGroup<Tangent::InverseDepth>()));
...
TEST(ErrorTermValidation, ValidateReprojectionError) {
  Tangent::SE3 host, target;
  Tangent::InverseDepth dinv(1);

  VariableContainer<Tangent::SE3, Tangent::InverseDepth> vc;
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    Tangent::ErrorTermValidator<RelativeReprojectionError> validator(errorTerm);
