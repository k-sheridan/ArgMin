...
TEST(Tangent, SparseBlockRowOperations) {
  SE3 pose;
  InverseDepth zinv;

  using SBR = Tangent::SparseBlockRow<Scalar<double>, Dimension<2>,
                                     Tangent::VariableGroup<SE3, InverseDepth>>;
...
TEST(Tangent, SparseBlockMatrixOperations) {
  SE3 pose;
  InverseDepth zinv;

  using SBM =
      Tangent::SparseBlockMatrix<Scalar<double>,
                                Tangent::VariableGroup<SE3, InverseDepth>>;
...
TEST(Tangent, BlockVector) {
  SE3 pose;
  InverseDepth zinv;

  using V = Tangent::BlockVector<Scalar<double>, Dimension<1>,
                                Tangent::VariableGroup<SE3, InverseDepth>>;
...
TEST(Tangent, GaussianPrior) {
  SE3 pose;
  InverseDepth zinv;

  using Prior = Tangent::GaussianPrior<Scalar<double>,
                                      Tangent::VariableGroup<SE3, InverseDepth>>;
  Prior prior;
...
TEST(Tangent, ErrorTermBasePointer) {
  using ET = Tangent::ErrorTermBase<Tangent::Scalar<double>, Tangent::Dimension<2>,
                                   Tangent::VariableGroup<SE3, SE3>>;
 ...
 */
TEST(Tangent, PSDSchurSolverSimple) {
  SimpleScalar ss1 = 1;
  SimpleScalar ss2 = 2;
  DifferentSimpleScalar dss1 = 5;

  using ErrorTermSet = Tangent::ErrorTermGroup<DifferenceErrorTerm>;

  using LS = Tangent::PSDSchurSolver<
      Scalar<double>, Tangent::LossFunction<Tangent::HuberLossFunction<double>>,
      ErrorTermSet, Tangent::VariableGroup<SimpleScalar, DifferentSimpleScalar>,
      Tangent::VariableGroup<SimpleScalar>>;
  ...
  using Prior = Tangent::GaussianPrior<
      Scalar<double>,
      Tangent::VariableGroup<SimpleScalar, DifferentSimpleScalar>>;
  Prior prior;
...
TEST(InverseDepth, BoundedUpdateTest) {
  ...
  Tangent::InverseDepth dinv(std::numeric_limits<double>::min() + 0.01);
